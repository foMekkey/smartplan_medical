...
class WarehouseDispatch(Document):
	...
	def validate_stock_availability(self):
		...
		if not pharma_warehouse.erpnext_warehouse:
			frappe.throw(_("المخزن {0} غير مربوط بمخزن ERPNext").format(frappe.bold(self.warehouse)))
		...
		for item in self.items:
			...
			if flt(available_qty) < flt(item.qty):
				frappe.throw(_(
					"الكمية المتاحة للصنف {0} في المخزن {1} ({2}) هي {3} فقط، والمطلوب {4}"
				).format(
					frappe.bold(item.item_code),
					frappe.bold(self.warehouse),
					frappe.bold(erpnext_warehouse),
	...
	def apply_fefo_logic(self):
		...
		for item in self.items:
			...
			if not has_batch_no:
				# Item doesn't use batches, skip FEFO logic
				continue
			...
			""", (item.item_code, erpnext_warehouse, today()), as_dict=1)
			
			if not batches:
				frappe.throw(_(
					"لا توجد دفعات متاحة للصنف {0} في المخزن {1} ({2})"
				).format(item.item_code, self.warehouse, erpnext_warehouse))
	...
	def create_delivery_note(self):
		...
		# Use make_delivery_note to properly link to Sales Order
...
def get_pending_tele_sales_orders(warehouse=None):
	...
	orders = frappe.get_all(
		"Tele Sales Order",
		filters=filters,
		# customer_name is a fetched/display-only field; requesting it directly in get_all
		# can trigger "Field not permitted in query". Fetch allowed base fields only
		# and populate customer_name explicitly.
		fields=["name", "customer", "order_date", "net_amount"],
		order_by="order_date desc"
	)

	# Populate customer_name safely
	for o in orders:
		try:
			o["customer_name"] = frappe.db.get_value("Pharma Customer", o.get("customer"), "legal_name") or o.get("customer")
		except Exception:
			o["customer_name"] = o.get("customer")
